...
export async function addNames(masteries: any) {
  for (var i = 0; i < masteries.length; i++) {
    var { championId } = masteries[i];
    var champion = championNames.filter(
      (champion) => champion.id == championId
    )[0];
    masteries[i].championName = champion.name;
    var nospaceName = champion.name.replace(/ /g, "");

    masteries[i].imageLocation = `../championicons/${nospaceName}_0.jpg`;
  }
  return masteries;
}
